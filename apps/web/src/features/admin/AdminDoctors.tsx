    ...
    <section className="mx-auto max-w-6xl">
      ...
      <form className="mt-5 rounded-md border border-border bg-surface p-4 shadow-sm" onSubmit={(event) => { event.preventDefault(); addDoctor(); }}>
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto]">
          ...
          <label className="text-sm font-medium text-text">Chuyên khoa<select className="mt-1 h-10 w-full rounded-md border border-border bg-surface px-3" onChange={(event) => setSpecialtyId(event.target.value)} value={specialtyId}><option value="">Chọn chuyên khoa</option>{mockStore.specialties.map((specialty) => <option key={specialty.id} value={specialty.id}>{specialty.name}</option>)}</select></label>
