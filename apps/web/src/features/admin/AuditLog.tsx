    ...
    <section className="mx-auto max-w-6xl">
      ...
      <fieldset className="mt-5 rounded-md border border-border bg-surface p-4 shadow-sm">
        ...
        <div className="mt-3 grid gap-3 sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto]">
          ...
          <label className="text-sm font-medium text-text">Hành động<select className="mt-1 h-10 w-full rounded-md border border-border bg-surface px-3" onChange={(event) => setAction(event.target.value)} value={action}><option value="">Tất cả hành động</option>{actions.map((item) => <option key={item} value={item}>{item}</option>)}</select></label>
