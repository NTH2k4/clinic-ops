...
export function mapPatient(record: ApiPatientRecord): Patient {
  ...
  return {
    ...
    ...(record.citizenIdNumber ? { citizenIdNumber: record.citizenIdNumber } : {}),
    ...(record.healthInsuranceNumber ? { healthInsuranceNumber: record.healthInsuranceNumber } : {}),
    ...(record.guardianName ? { guardianName: record.guardianName } : {}),
    ...(record.guardianPhone ? { guardianPhone: record.guardianPhone } : {}),
    ...(record.identityDocumentType ? { identityDocumentType: record.identityDocumentType } : {}),
    ...(record.maskedCitizenIdNumber ? { maskedCitizenIdNumber: record.maskedCitizenIdNumber } : {}),
    ...(record.maskedHealthInsuranceNumber ? { maskedHealthInsuranceNumber: record.maskedHealthInsuranceNumber } : {}),
