...
import { serviceCreateSchema, serviceListQuerySchema, serviceUpdateSchema } from "./catalog.dto";
...
export class ServicesController {
  ...
  async list(@Req() request: AuthenticatedRequest, @Query() rawQuery: Record<string, unknown>) {
    const query = parseSchema(serviceListQuerySchema, rawQuery);
    const result = await this.catalog.listServices({ ...query, includeRequestedStatus: request.currentUser.role === UserRole.admin });
