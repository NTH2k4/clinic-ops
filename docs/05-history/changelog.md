...
- Hoàn thiện Phase 2 Account Administration verification: bổ sung Playwright API-mode cho patient registration vào booking workspace, password change bắt buộc đăng nhập lại và admin lock/unlock account smoke; dữ liệu patient dùng email/số điện thoại sinh duy nhất, API-mode teardown reset DB về seeded baseline sau suite, không ghi session token hay mật khẩu vào test/docs. Full API gate pass (`7/7` unit suites, `41/41` tests; `10/10` E2E suites, `89/89` tests); full Web gate pass (`16/16` files, `141/141` tests; mock Playwright `9/9`; API-mode Playwright `8/8`).
