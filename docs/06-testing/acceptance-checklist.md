...
- [ ] Scheduling production data smoke is blocked until manual Render deploy of latest `main`. Production catalog/scheduling baseline was missing after `49a4ff2a`: `/services`, `/doctors`, `/specialties`, `/doctor-schedules` and `/appointments` returned total `0`; `/availability/slots?serviceId=service-general...` returned `404 service was not found`.
