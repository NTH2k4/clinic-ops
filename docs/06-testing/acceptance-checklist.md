...
- [x] Review fix: smoke script revokes its temporary login session in cleanup. RED: `node --test scripts/production-smoke.test.mjs` initially failed because `runProductionSmoke` did not exist. GREEN: the same command passed `1/1` test and production smoke still passed.
