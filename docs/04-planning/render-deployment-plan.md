...
- `startCommand`: runs `npm run start --prefix apps/api` directly.
- API startup creates only missing demo users when `SERVE_WEB_APP=true`; it never changes an existing demo user's password hash, role, or account status.
